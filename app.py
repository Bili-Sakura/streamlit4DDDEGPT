...
if cols[1].button("run agent"):
    ...
    for d in agent.run_stream():
        ...
        if d["status"] == 1:
            ...
            text = ""
            chat_box.insert_msg(
                Markdown(text, title=d["text"], in_expander=True, expanded=True)
            )
        elif d["status"] == 2:
            text += d["llm_output"]
